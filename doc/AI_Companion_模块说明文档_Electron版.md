...
AI Companion 是一款本地优先的个人 AI 桌面助手，面向 Windows 和 macOS。系统在用户明确授权和隐私规则约束下，保存用户的对话、桌面活动、长期记忆、用户修订和目标任务，并利用这些信息提供持续、个性化且可追溯的 AI 对话体验。

产品核心不是单次问答，而是建立以下长期闭环：

用户产生对话或活动记录 → 系统识别其中可能长期有价值的信息 → 生成候选记忆 → 用户确认、纠正、否定或删除 → 系统在后续对话中检索相关记忆 → AI 基于最小必要上下文继续服务用户 → 对话结果进一步沉淀为目标和任务。
...
| 本地事实优先 | 对话、活动、记忆、人物理解、目标、任务和审计数据以本地数据库为事实来源，缓存与索引不是独立事实源 |
...
**首版必须形成以下闭环**：Windows 和 macOS 可安装运行；Electron 稳定启动和管理本地服务；可配置至少一种外部模型；多会话流式对话；从对话生成候选记忆；用户审查、确认、纠正、否定和删除记忆；后续对话使用有效记忆；查看基础桌面活动；开启、暂停和配置隐私规则；创建和管理目标任务；导出、备份和删除本地数据；展示关键能力状态和错误原因。
...
| Vue 前端界面 | 提供对话、记忆、活动、目标、设置和数据管理等用户入口 | 统一交互规范、完整状态表达、错误可恢复 |
...
- Electron 运行时不负责：记忆内容判断、人物理解推断、提示词业务编排、目标任务业务规则、数据库领域事务、具体模型供应商业务逻辑。
...
| 人物理解与行为理解 | 业务层 | 从已确认信息形成用户理解 | 观察、洞见、人物侧写、行为统计 | 会话提取、后台任务、持久化 |
| 目标与任务 | 业务层 | 管理目标、任务与执行状态 | 可追踪行动项 | 持久化、个性化对话（建议来源） |
| 持久化与索引 | 数据层 | 保存事实数据并维护检索索引 | 本地数据与索引 | 无（最底层） |
| 数据治理 | 业务与数据层 | 管理删除、保留、导出、备份与恢复 | 可控数据生命周期 | 全部数据生产模块、持久化 |
...
- 用户可查看并操作应用版本与更新状态；数据库恢复等维护场景进入受控停服状态。
- 平台能力状态对上层统一可见：前台应用、窗口标题（受权限限制）、用户空闲状态、系统通知、权限申请结果。
- 模型密钥和数据库密钥保存到操作系统安全存储。

**设计边界**：只管理运行状态和平台事实，不承担对话、记忆、人物理解和检索业务；不决定业务是否生成记忆或人物理解。

**依赖与协作**：被前端界面（经安全通信）和活动采集依赖；本地服务启动/停止/健康检查由其编排；数据治理的恢复流程要求其进入维护停服状态。
...
- 用户通过统一的侧边栏导航访问全部页面：今日概览、AI 对话、活动记录、长期记忆、用户理解、目标任务、模型设置、隐私设置、数据管理、系统状态。
...
- 用户可创建、选择、重命名和删除会话，进行流式对话，并可中止正在生成的回复、重新生成失败或不满意的回复。
- 系统自动保存用户消息和助手回复，记录模型调用结果和失败状态。
- 系统将已确认的人物理解注入系统提示词，并调用检索模块获取相关长期记忆，组装最小必要上下文。
- 回复保存本次实际使用的记忆引用，来源可追溯。
- 用户可收藏高质量的助手回复，并可将收藏内容一键采纳为候选记忆或生成目标任务建议。
- 用户可手动触发会话级提取，系统以水位线标记提取进度，避免重复提取。

**设计边界**：不实现底层检索算法，不直接依赖具体模型供应商，记忆提取不阻塞主回复。

**依赖与协作**：依赖模型与配置（模型调用）、检索与上下文组装（记忆召回）、人物理解（侧写注入）、记忆管理和目标与任务（收藏采纳）；被前端应用框架页面调用。

**开发关注点**：

- 流式回复必须支持中止与重试，失败状态可识别。
- 同会话并发生成需要互斥或明确排队，防止消息顺序错乱。
- 收藏、采纳、建议等派生操作失败不影响对话主流程。
...
- 用户可主动创建记忆，也可从对话、助手回复和主动选择的活动记录生成候选记忆。
- 系统校验候选记忆的内容、类型和重要性，保存记忆与来源证据的关联。
- 用户可确认、纠正、否定、删除记忆，可查看记忆来源和修订历史，可合并和去重相似记忆。
...
- 系统记录应用名称、窗口标题、活动起止时间、时长、空闲状态、采集来源和隐私处理结果。
...
**依赖与协作**：依赖跨平台适配（前台应用、窗口标题、空闲状态、权限）；被行为统计、数据治理和审计依赖；活动记录落入持久化层。
...
**定位**：该模块从用户对话中提取可追溯观察，通过自动反思形成洞见，再汇编为连贯的人物侧写；行为统计独立提供时间窗口内的活动与对话模式。
...
**定位**：该模块将 AI 理解和用户意图转化为可追踪的目标和任务。

**核心功能**：

- 用户可创建、修改、删除目标和任务，设置描述、日期、优先级和状态，将任务关联到目标或作为独立任务。
- 用户可查看目标进度。
- 对话中系统可生成任务建议，页面明确展示“建议”状态，用户确认后才创建目标或任务。

**设计边界**：首版只管理本地目标和任务，不自动发送外部消息、修改其他应用数据、创建第三方日历事件、操作文件或执行系统命令；AI 建议与已执行操作必须明确区分。

**依赖与协作**：依赖持久化；被个性化对话（任务建议来源）引用；可为后续对话提供上下文。

**开发关注点**：

- 建议状态与已确认状态严格分离，未确认的建议不得影响业务统计。
- 任务状态流转需有明确规则，避免悬空状态。
- 删除目标时需要明确关联任务的处置（保留独立任务或级联处理）。
...
- 保存会话消息、活动记录、隐私规则、长期记忆及来源与修订历史、人物理解及来源与修订历史、目标任务、模型非敏感配置、后台任务、审计记录、模型调用记录、导出备份记录。
- 提供事务管理、外键关系、唯一约束、级联删除和数据迁移。
- 维护全文索引（FTS5）与向量索引（嵌入向量以 BLOB 存入记忆行，FastEmbed + ONNX 本地推理生成，NumPy 余弦相似度检索）。
- 提供数据文件加密、一致性检查、数据库健康状态和索引重建。

**事实与派生关系**：对话消息、活动记录、用户确认后的记忆、用户修订后的人物理解、目标任务、用户配置属于事实或用户可控数据；全文索引、向量索引、部分统计缓存、搜索排序结果属于可重建派生结构。
...
**定位**：该模块管理本地数据从产生、审查、保留到删除、导出、备份和恢复的完整生命周期。

**核心功能**：

- 用户可删除单条活动记录、会话与消息、记忆与人物理解、目标任务，系统同步处理关联证据、派生理解和索引。
- 用户可配置数据保留期限，系统定时执行数据清理。
- 用户可导出个人数据，导出结果包含导出时间、数据范围、文件位置、状态、敏感内容说明和加密信息。
- 用户可创建和管理备份、执行受控数据库恢复、清除全部本地数据。
- 系统记录不含敏感正文的审计信息。

**删除原则**：删除原始事实时同步处理相关记忆来源、派生人物理解、对话引用、全文索引、向量索引、行为统计和其他关联数据，不能只删除当前页面对应的一张表。

**恢复原则**：数据库恢复期间暂停新对话写入、活动采集、后台任务、索引写入、自动清理和备份；恢复失败时保留原数据库并提供回滚。

**依赖与协作**：依赖持久化，并覆盖全部数据生产模块（对话、记忆、活动、人物理解、目标任务）的数据；恢复流程要求桌面运行时进入维护停服。

**开发关注点**：

- 一致性删除必须通过证据链和派生关系递归定位，不能按页面维度删除。
- 导出必须明确标注是否含敏感内容与是否加密。
- 恢复期间所有写入通道必须真正暂停，不能只暂停页面入口。
- 清除全部数据前必须有二次确认与审计记录。
...
**职责**：处理不应阻塞用户主流程的异步派生工作，包括会话级提取（含人物理解演化）、记忆向量更新、人物理解刷新、行为统计、数据保留清理、自动备份、索引重建和失败任务重试。
...
说明：事实数据是唯一事实来源，索引与统计可重建；删除事实时必须同步重建或清理派生结构。
...
### 8.4 将对话内容沉淀为目标、记忆或收藏

1. 用户选中一条助手回复。
2. 用户选择收藏、采纳为记忆或生成任务建议。
3. 采纳为记忆时，系统以该回复为来源生成候选记忆，进入用户确认流程。
4. 生成任务建议时，页面明确展示“建议”状态，用户确认后才创建任务。
5. 目标与任务进入本地管理，后续对话可引用有效目标与任务。
...
8. 删除事实数据时同步处理证据、派生数据和索引；恢复期间暂停全部写入通道。
