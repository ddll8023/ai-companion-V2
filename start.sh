...
# AI Companion 一键启动脚本
# 用法:
#   ./start.sh             启动 后端 + 前端 (开发模式，浏览器访问)
#   ./start.sh --all       启动 后端 + 前端 + Electron (完整模式)
#   ./start.sh --electron  启动 前端 + Electron (Electron 自行管理后端)
